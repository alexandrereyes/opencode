...
`ctx.session.scan()` returns ID-ordered session metadata without decoding transcripts. The default limit is 200 and the
maximum is 1000.
...
effect: (ctx) =>
  Effect.gen(function* () {
    ...
    const page = yield* sessions.scan({ limit: 500, archived: false })
    yield* Effect.logInfo("session metadata", { count: page.data.length })
  }),
```

Use `sessionID` to select one session. Omit `archived` to include all sessions, pass `false` for unarchived sessions, or
`true` for archived sessions. `messageAt` reports user or assistant activity; `completionAt` reports only durable succeeded
or failed executions.

Use `parentID` to select direct children, or `parentID: null` to select root sessions.

```ts
const children = yield * ctx.session.scan({ parentID: sessionID })
```

### Live pending requests

`ctx.request.pending()` snapshots raw pending permissions and forms from already-live Locations without starting historical
Locations. The snapshot is best-effort rather than atomic across Locations.

```ts
effect: (ctx) =>
  Effect.gen(function* () {
    const requests = ctx.request
    const snapshots = yield* requests.pending()
    yield* Effect.logInfo("pending requests", { locations: snapshots.length })
  }),
```
...
Archive exactly one session with the native session capability. It stops active execution, waits for idle, closes the model
transport, and records the archive event without deleting history. It does not archive descendants; scan and archive
descendants explicitly when the plugin owns a tree policy.
...
Use `update` for an atomic read-modify-write shared by every Location running the same plugin ID. Its callback is
synchronous and returns the next JSON value together with a local result. If it throws, the stored value is unchanged.

```ts
effect: (ctx) =>
  ctx.storage.update("counter", (current) => {
    const next = typeof current === "number" ? current + 1 : 1
    return [next, next]
  }),
```

Use `adoptLegacy` only when moving an existing unnamespaced key into plugin storage. Adoption uses the same key name,
preserves an existing destination, and runs once even if that destination is later removed.

```ts
effect: (ctx) => ctx.storage.adoptLegacy("settings").pipe(Effect.asVoid),
```
...
Plan derived Session history rollback with `revert.plan`. The shared contract is exported from
`@opencode/plugin/session-revert`.
...
function selectRevertPlan(facts: CausalRevert.Facts): CausalRevert.Plan {
  return { participants: [], origins: [], pendingOrigins: [], discardedSessionIDs: [] }
}
```

Treat `facts` as an immutable snapshot and keep the planner deterministic and free of I/O. OpenCode activates the root
Session's Location before collecting facts, invokes the hook outside Session locks, and may invoke it again if provenance
changes. The hook has no `providerID` option.

OpenCode validates the returned plan before invalidating jobs, interrupting Sessions, restoring snapshots, or persisting
state. With no planner, a new revert is root-only. Already staged participant plans remain durable and continue through
clear, commit, and automatic commit from a new prompt, synthetic input, or compaction after the plugin unloads.
