...
Use `ctx.session.scan()` to inspect bounded session metadata without loading transcripts. Pages are ordered by session ID;
the default limit is 200 and the maximum is 1000.

```ts
const page = await ctx.session.scan({ limit: 500, archived: false })
...
Use `sessionID` to select one session. Omit `archived` to include all sessions, pass `false` for unarchived sessions, or
`true` for archived sessions. `messageAt` is the latest user or assistant message timestamp, and `completionAt` is the
latest durable succeeded or failed execution timestamp.

Use `parentID` to select direct children, or `parentID: null` to select root sessions.

```ts
const children = await ctx.session.scan({ parentID: sessionID })
```

### Live pending requests

Use `ctx.request.pending()` to read raw pending permissions and forms grouped by Location.
...
The snapshot examines only already-live Locations and never starts historical Locations. It is best-effort and is not an
atomic transaction across Locations.
...
Rename, interrupt, wait for, or archive a session.
...
`archive` affects exactly the target session: it stops active execution, waits for idle, closes its model transport, and
records the native archive event without deleting history. It does not archive descendants; scan and archive descendants
explicitly when your plugin owns a tree policy.
...
interface SessionContext {
  ...
  scan(input?: SessionScanInput): Promise<SessionScanPage>
  archive(input: { sessionID: string }): Promise<void>
}
```
...
Update one value atomically when multiple plugin instances may write it. The callback is synchronous and returns the
next JSON value plus a local result. If it throws, the stored value is unchanged.

```ts
const count = await ctx.storage.update("counter", (current) => {
  const next = typeof current === "number" ? current + 1 : 1
  return [next, next]
})
```

Adopt a legacy unnamespaced key once when moving existing data into plugin storage. The source must have the same key;
an internal marker prevents a deleted destination from importing it again.

```ts
const imported = await ctx.storage.adoptLegacy("settings")
```
...
interface StorageContext {
  ...
  update<A>(key: string, update: (current: Json | undefined) => readonly [next: Json, result: A]): Promise<A>
  adoptLegacy(key: string): Promise<boolean>
}
...
Use `revert.plan` to choose the derived Sessions and asynchronous origins included when a Session revert is staged.
Import the canonical facts and plan contract from `@opencode/plugin/session-revert`.

```ts
import { CausalRevert } from "@opencode/plugin/session-revert"

function selectRevertPlan(facts: CausalRevert.Facts): CausalRevert.Plan {
  return {
    participants: [],
    origins: [],
    pendingOrigins: [],
...
await ctx.session.hook("revert.plan", (event) => {
  event.plan = selectRevertPlan(event.facts)
})
```

`facts` is an immutable snapshot containing the root boundary, Session ancestry and Locations, subagent input assignments,
their current message or inbox state, and tool origins. The callback replaces `plan`; it should be deterministic and must
not perform I/O or other side effects because OpenCode may invoke it again when provenance changes.

- The root Session's Location plugins are ready before facts are collected, and only that Location plans the revert.
- Planning runs outside Session locks and has no provider-scoping option. OpenCode validates the result before interruption,
  job invalidation, snapshots, or persistence.
- Without a registered planner, a newly staged revert affects only the root Session.
- Staged participant plans are durable. Clear, commit, and automatic commit from a new prompt, synthetic input, or
  compaction continue to use the stored plan after the planner unloads.
...
##### HTTP failure metadata

Use optional `event.http` to inspect the failed physical attempt's HTTP response after `http.response` hooks.
For example, preserve a provider's explicit retry veto:

```ts
await ctx.session.hook("retry", (event) => {
  if (event.http?.headers["x-should-retry"] === "false") {
    event.decision = { retry: false }
  }
})
```

- `http` contains the original cause's URL, status and response headers. The object and its copied headers are readonly and frozen.
- It is runtime-only: these fields are not added to the public or persisted `event.error` or retry-scheduled error.
- Failures without an observed HTTP response, including connection timeouts, omit `http`. Older hosts may also omit it; retain native policy when the metadata needed by your hook is absent.
- The URL can omit separately stored query parameters. Do not correlate it by comparing the complete serialized Web Request URL.
- Carry response-specific metadata through response hooks rather than a shared last-error cache; auxiliary calls and separate sessions can fail concurrently. Scope provider-specific policy to the matching provider and account.
...
interface SessionHooks {
  ...
  "revert.plan": CausalRevert.PlanEvent
}
...
interface SessionHookContext {
  hook<Name extends keyof SessionHooks>(
    ...
    options?: SessionHooks[Name] extends { readonly model: unknown } ? { providerID?: string } : never,
