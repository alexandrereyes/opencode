...
test("missing or stale measurements suppress the whole pool percentage", () => {
  ...
  expect(subscriptionPool([])).toEqual({ total: 0, measured: 0, ready: 0, remaining: null, observedAt: null })
})
...
test("cooldown and capacity affect availability, while quota and oldest observation describe the pool", () => {
  ...
  ).toEqual({ total: 2, measured: 2, ready: 0, remaining: 45, observedAt: Date.parse(account.observedAt) })
})
