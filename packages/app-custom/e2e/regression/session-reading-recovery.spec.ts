...
for (const mode of ["recover", "cancel", "exhausted"]) {
  test(`${mode} a bookmark after reconnect replaces history with the latest page`, async ({ page }) => {
    ...
    const transport = await setup(page, {
      pageMessages: (id, limit, before) => {
        ...
        const start = Math.max(0, end - Math.min(limit, mode === "exhausted" ? 6 : 10))
    ...
    if (mode === "exhausted") {
      await expect.poll(() => state.pages).toBe(3)
      await expect.poll(() => distanceFromEnd(page)).toBeLessThan(2)
      await expect(page.getByText("Reading answer 19", { exact: true })).toBeInViewport()
      return
    }
...
test("invalid and removed hashes preserve paused reading", async ({ page }) => {
  await setup(page)
  await readDeep(page)
  const before = await deepGeometry(page)
  await navigate(page, sourceID, "#missing-reading-anchor")
  await expect(page).toHaveURL(/#missing-reading-anchor$/)
  await expect.poll(async () => Math.abs((await deepGeometry(page)).offset - before.offset)).toBeLessThan(3)
  await navigate(page, sourceID)
...
test("rapid A to B to A preserves the owning anchor and follow intent", async ({ page }) => {
  await setup(page)
  await readDeep(page)
  const before = await deepGeometry(page)
  await navigate(page, targetID)
