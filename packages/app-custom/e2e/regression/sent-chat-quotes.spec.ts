...
for (const variant of [
  ...
  for (const scenario of ["with-text", "quote-only", "multiple"]) {
    test(`sent quotes ${scenario} ${variant.width}px ${variant.scheme}`, async ({ page }, info) => {
      ...
      const previews = bubble.locator('[data-slot="user-message-quote-preview"]')
      await expect(previews).toHaveCount(quotes.length)
      await expect(previews.filter({ hasText: `Quoted from an earlier message: ${longQuote.comment}` })).toHaveCount(1)
      await expect(bubble.locator('[data-component="user-message-quote"]')).toHaveCount(0)
      ...
      const excerpt = quote.locator("blockquote")
      ...
      await expect
        .poll(() =>
          excerpt.evaluate((element) => element.clientHeight / Number.parseFloat(getComputedStyle(element).lineHeight)),
        )
        .toBe(4)
      ...
      await expect
        .poll(() =>
          excerpt.evaluate(
            (element) => element.clientHeight > 4 * Number.parseFloat(getComputedStyle(element).lineHeight),
          ),
        )
        .toBe(true)
      await expect(quote.getByText(longQuote.comment, { exact: true })).toBeVisible()
...
test("retains quote disclosure through virtual remounts and cached session switches", async ({ page }) => {
  ...
  await expect(collapse).toHaveAttribute("aria-expanded", "true")
  // The expanded bubble has its own contained scroller, so wheel outside it.
  await page.getByText("Earlier message 79", { exact: true }).hover()
  await page.mouse.wheel(0, -100000)
  await expect(page.getByText("Earlier message 0", { exact: true })).toBeInViewport()
  await expect(bubble).toHaveCount(0)
  await page.getByRole("button", { name: "Jump to latest", exact: true }).click()
  await expect(collapse).toHaveAttribute("aria-expanded", "true")
  await expect(bubble.locator("blockquote")).toHaveCSS("-webkit-line-clamp", "none")
...
test("isolates mixed-direction quotes and keeps disclosure keyboard accessible in RTL", async ({ page }) => {
  ...
  await expect(bubble.locator("blockquote")).toHaveCSS("direction", "rtl")
  await expect(expand).toHaveCSS("border-right-width", "2px")
  await expand.focus()
  await expand.press("Space")
  await expect(bubble.getByRole("button", { name: "Collapse quote", exact: true })).toBeFocused()
  await expect(bubble.locator("blockquote")).toHaveText(quote.text)
