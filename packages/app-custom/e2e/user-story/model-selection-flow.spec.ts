...
test("creates a session in a new project and selects its model", async ({ page }) => {
  ...
  const pathInput = picker.getByRole("combobox")
  // Without a reported home directory the explorer starts at the current Location.
  await expect(pathInput).toHaveValue("C:/OpenCode/NewProject/")
  ...
  await expect(pathInput).toHaveValue("C:/OpenCode/")
  const directoryItem = picker.getByRole("option").filter({ hasText: "NewProject" })
  await expect(directoryItem).toBeVisible()
  await expect(directoryItem).toHaveAttribute("aria-selected", "true")
  await pathInput.fill("C:/OpenCode/NewProject")
  const add = picker.getByRole("button", { name: "Add project", exact: true })
