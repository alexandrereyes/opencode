...
story("keeps the default thinking variant visible and muted", async ({ mount, page }) => {
  const component = await mount("opencode-composer-flow--model-and-variant")
  ...
  const muted = await control.evaluate((element) => getComputedStyle(element).color)

  await control.click()
  await page.getByRole("menuitemradio", { name: "high", exact: true }).click()
  await expect(control).toHaveText("high")
  await expect(control).not.toHaveCSS("color", muted)
})

story("reaches thinking, model, and agent controls by keyboard in toolbar order", async ({ mount, page }) => {
  const component = await mount("opencode-composer-flow--model-and-variant")
  const input = component.getByRole("textbox", { name: "Prompt", exact: true })
  const variant = component.getByRole("button", { name: "Choose model variant" })

  await input.click()
  // Tab through Add to the thinking trigger, which now leads the right-aligned controls.
  for (let count = 0; count < 2; count++) await page.keyboard.press("Tab")
  await expect(variant).toBeFocused()
  await page.keyboard.press("Enter")
  ...
  await expect(component.locator('[data-action="composer-model"]')).toBeFocused()
})
