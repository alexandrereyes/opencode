...
import type { FileSearchHandle } from "@opencode/session-ui-custom/file"
import { useFileComponent } from "@opencode/ui-custom/context/file"
import { cloneSelectedLineRange, previewSelectedLines } from "@opencode/session-ui-custom/pierre/selection-bridge"
import { createLineCommentControllerV2 } from "@opencode/session-ui-custom/v2/line-comment-annotations-v2"
import { sampledChecksum } from "@opencode/util/encode"
import { LineCommentOverflowIcon } from "@opencode/ui-custom/line-comment"
    ...
    <div class="relative h-full min-h-0 flex flex-col">
      <Switch>
        <Match when={state()?.loaded ? state()?.content : undefined}>
          {(value) => (
            <Show
              when={value().type === "binary" || artifactKind(path() ?? "") !== "text"}
              fallback={codeView(contents())}
            >
              <ArtifactView path={path() ?? ""} content={value()} source={codeView(contents())} />
            </Show>
          )}
        </Match>
