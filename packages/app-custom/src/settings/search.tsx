...
import { settingsSearchIndex, settingsSearchProjects } from "./search-index"
...
export function SettingsSearch() {
  ...
  const inventory = createMemo(() =>
    servers.list.map((server) => {
      ...
      return {
        ...
        name: serverName(server) || ServerConnection.key(server),
        connected: context.sdk.connection.status() === "connected",
        projects: settingsSearchProjects(context.projects.list(), context.sync.data.project),
      }
    }),
  )
  ...
  const group = (item: SettingsSearchResult) => {
    ...
    if (!item.server || servers.list.length > 1) return item.owner
