...
export function rankSettings(query: string, items: SettingsSearchResult[], origin: SettingsView) {
  ...
  return items
    .flatMap((item) => {
      ...
      const search = name ? normalize(` ${value} `.replace(` ${name} `, " ")) : value
      if (!search) return []
      ...
      const fuzzy = fuzzysort.single(search, title)?.score ?? 0
      // Context qualifies a setting match; a project name alone should not return all its pages.
