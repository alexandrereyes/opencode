...
import { type UiI18n, useI18n } from "@opencode/ui-custom/context/i18n"
import { BasicTool, GenericTool } from "../components/basic-tool"
import { FileIcon } from "@opencode/ui-custom/file-icon"
import { Icon, type IconProps } from "@opencode/ui-custom/icon"
...
export function CurrentFileToolGroup(props: {
  ...
  const files = createMemo((previous: { key: string; toolID: string; value: unknown }[]) => {
    const next = props.tools.flatMap((tool) => {
      ...
      if (tool.name !== "write" || typeof input.content !== "string") return []
      return [
        {
          key: `${tool.id}:0`,
          toolID: tool.id,
