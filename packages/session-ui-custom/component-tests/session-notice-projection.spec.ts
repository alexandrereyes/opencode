...
story("renders current protocol notices in CLI order", async ({ mount, page }) => {
  ...
  await expect(notices.nth(0)).toContainText("Agent changed")
  await expect(notices.nth(0)).toContainText("Explore")
...
story("renders a compaction summary while it streams and after completion", async ({ mount }) => {
  ...
  await expect(compaction.getByText("Compacting", { exact: true })).toHaveCount(2)
  ...
  await expect(compaction.getByText("Compacting", { exact: true })).toHaveCount(0)
